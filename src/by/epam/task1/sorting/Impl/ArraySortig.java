...
import by.epam.task1.sorting.ArraySortingInterface;
...
public class ArraySortig implements ArraySortingInterface {
    static Logger logger = LogManager.getLogger();


    @Override
    public double[] bubbleSortfFromBig(Massiv massiv) throws ArrayException {
        double[] dblArray = massiv.getArray();
        int i, j;
        double tempInt;
        if (dblArray.length <= 2) {
            logger.info("There is too small Massiv " + dblArray);
            throw new ArrayException("There is too small Massiv " + dblArray);
        }
